...
| `CALIBRATION_INCOMPLETE` | Any required condition failed or could not be evaluated. The `conditions` map names each one with a reason. |
| `CALIBRATION_USABLE` | **Every** required condition passes. The force channel is characterised over the calibrated range. **This is not campaign readiness**, which needs the full physical-readiness checklist and a reviewer. |

### Required conditions (2026-09-24)

`CALIBRATION_USABLE` is granted only when all of these pass. A diagnostic that is computed but absent
from this list cannot silently fail to matter — that was a defect in the first version, where a badly
curved calibration returned `LACK_OF_FIT_SIGNIFICANT` and `CALIBRATION_USABLE` at the same time.

`no_fatal_blockers` · `no_advisory_blockers` · `model_adequate` · `zero_observations` ·
`gravity_uncertainty` · `covers_campaign_range` · `identity_complete` · `held_out_scored` · `reviewed`

Two of these need explanation. **Zero observations and the gravity uncertainty must be measured or
explicitly declared negligible with a stated basis** (`zero_drift_declared_negligible`,
`u_g_declared_negligible`); a defaulted zero is an assumption, not a datum. **Held-out points** must be
supplied in `verification_points`; they take no part in the fit, because scoring a fit on the data that
produced it measures nothing about prediction.

### The lack-of-fit decision

The first version compared a mean-square ratio against a fixed cutoff of 5 and called the result
significant or not. That cutoff was arbitrary, and a classical F quantile would not have been correct
either: this is a weighted errors-in-variables fit, not the fixed-x homoscedastic ordinary-least-squares
model the classical test assumes. The null distribution is now generated by **parametric bootstrap under
the fitted model**, and the decision uses a declared `alpha`. Pure error is pooled **within a
(level, direction) cell**, so a systematic ascending/descending offset is reported as a direction effect
instead of being absorbed into noise.
