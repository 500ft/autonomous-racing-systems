...
ZERO_RESIDUAL_N = 1e-12          # below this a residual set counts as exactly zero, avoiding 0/0
LOF_ALPHA = 0.01                 # declared significance level for the lack-of-fit decision
LOF_NULL_DRAWS = 300             # parametric-bootstrap draws for the null distribution


def _lof_statistic(pts, resid):
    """Lack-of-fit versus pure-error decomposition.

    Pure error is measured *within a (level, direction) cell*, not within a level. Pooling the
    ascending and descending readings at one level would absorb a systematic hysteresis offset into
    "noise" and thereby hide the very inadequacy this statistic exists to detect."""
    cells, by_level = {}, {}
    for p, r in zip(pts, resid):
        lv = round(p["nominal_mass_kg"], 6)
        cells.setdefault((lv, p.get("direction")), []).append(r)
    ...
    for rs in cells.values():
        if len(rs) >= 2:
            ...
            pure_dof += len(rs) - 1
    lof_ss, groups = 0.0, 0
    for rs in by_level.values():
        m = statistics.fmean(rs)
        lof_ss += len(rs) * m * m
        groups += 1
    lof_dof = max(0, groups - 2)
    if pure_dof < 1 or lof_dof < 1:
        return None, pure_ss, pure_dof, lof_ss, lof_dof
    pure_ms, lof_ms = pure_ss / pure_dof, lof_ss / lof_dof
    if pure_ms <= ZERO_RESIDUAL_N ** 2 and lof_ms <= ZERO_RESIDUAL_N ** 2:
        return 0.0, pure_ss, pure_dof, lof_ss, lof_dof      # exact fit: 0/0 is not a failure
    if pure_ms <= 0:
        return float("inf"), pure_ss, pure_dof, lof_ss, lof_dof
    return lof_ms / pure_ms, pure_ss, pure_dof, lof_ss, lof_dof


def lack_of_fit(pts, rec, fit, draws=LOF_NULL_DRAWS, seed=20260924):
    """Is the departure from a straight line larger than the measurement's own repeatability?

    The classical F test assumes a fixed-x, homoscedastic, ordinary-least-squares model. This fit is
    a weighted errors-in-variables fit, so an F quantile borrowed from that model would not have the
    stated size. The null distribution is therefore generated by parametric bootstrap **under this
    model**: simulate from the fitted affine relation with the declared per-point uncertainties,
    recompute the statistic, and read the p-value off the simulated distribution (JCGM 101 in
    spirit). `alpha` is declared, not implied."""
    stat, pure_ss, pure_dof, lof_ss, lof_dof = _lof_statistic(pts, fit["residuals_N"])
    base = dict(pure_error_sd_N=(math.sqrt(pure_ss / pure_dof) if pure_dof >= 1 else None),
                lack_of_fit_statistic=stat, pure_error_dof=pure_dof, lack_of_fit_dof=lof_dof,
                alpha=LOF_ALPHA, null_draws=draws,
                method="parametric bootstrap under the fitted errors-in-variables model; no classical "
                       "F quantile is used because this model does not meet that test's assumptions",
                note="R^2 is deliberately not reported: discouraged as a calibration metric")
    if stat is None:
        return base | dict(verdict="NOT_EVALUABLE",
                           reason="needs replicates within at least one (level, direction) cell and "
                                  "3 or more distinct levels")
    if stat == 0.0:
        return base | dict(verdict="LACK_OF_FIT_NOT_SIGNIFICANT", p_value=1.0,
                           reason="residuals are zero to within tolerance; an exact fit is not a failure")
    ...
    for _ in range(draws):
        sim = []
        for p in pts:
            ux = p["counts_sd"] / math.sqrt(p.get("n", 1))
            uy = u_force(p, rec)
            x = p["counts_mean"] + rng.gauss(0.0, ux)
            sim.append((x, a + b * p["counts_mean"] + rng.gauss(0.0, uy)))
        try:
            f = york_fit([s[0] for s in sim], [s[1] for s in sim],
                         [p["counts_sd"] / math.sqrt(p.get("n", 1)) for p in pts],
                         [u_force(p, rec) for p in pts])
        except (ValueError, ZeroDivisionError):
            continue
        s_null, *_ = _lof_statistic(pts, f["residuals_N"])
        if s_null is not None and s_null >= stat:
            null += 1
    p_value = (null + 1) / (draws + 1)          # never reports exactly zero
    return base | dict(verdict=("LACK_OF_FIT_SIGNIFICANT" if p_value < LOF_ALPHA
                                else "LACK_OF_FIT_NOT_SIGNIFICANT"),
                       p_value=p_value,
                       reason=f"simulated p = {p_value:.4g} against declared alpha = {LOF_ALPHA}")


def direction_effect(pts, fit):
    """Mean residual by loading direction, kept out of the pure-error term on purpose."""
    by = {}
    for p, r in zip(pts, fit["residuals_N"]):
        by.setdefault(p.get("direction"), []).append(r)
    means = {k: statistics.fmean(v) for k, v in by.items() if v}
    if len(means) < 2:
        return dict(separation_N=None, means_N=means, note="only one loading direction present")
    return dict(separation_N=max(means.values()) - min(means.values()), means_N=means,
                note="systematic ascending/descending offset; reported apart from repeatability so it "
                     "cannot be pooled into noise")
...
def analyse(rec: dict) -> dict:
    ...
    out["fit_quality"] = lack_of_fit(pts, rec, fit)
    out["direction_effect"] = direction_effect(pts, fit)
    ...
    out["held_out"] = held_out_check(rec, fit)
    out["conditions"] = conditions(rec, out, blockers, unresolved)
    failed = sorted(k for k, v in out["conditions"].items() if v["status"] == "fail")
    unevaluable = sorted(k for k, v in out["conditions"].items() if v["status"] == "unevaluable")
    out["failed_conditions"], out["unevaluable_conditions"] = failed, unevaluable
    out["verdict"] = "CALIBRATION_USABLE" if not failed and not unevaluable else "CALIBRATION_INCOMPLETE"
    out["verdict_note"] = ("A usable verdict describes this force channel only. It is not campaign "
                           ...
                           "Every required condition must pass; a condition that cannot be evaluated "
                           "blocks the verdict rather than being assumed satisfied.")
    return out


def held_out_check(rec, fit):
    """Score reserved points that took no part in the fit. Fitting everything scores nothing."""
    pts = rec.get("verification_points") or []
    if not pts:
        return dict(status="absent", n=0,
                    note="no verification_points supplied; the fit was scored only on the data that "
                         "produced it, which measures nothing about prediction")
    b, a = fit["slope_N_per_count"], fit["intercept_N"]
    resid = [applied_force(p, rec) - (a + b * p["counts_mean"]) for p in pts]
    worst = max(resid, key=abs)
    return dict(status="present", n=len(pts), residuals_N=[round(r, 6) for r in resid],
                max_abs_residual_N=abs(worst), rms_residual_N=math.sqrt(statistics.fmean(r * r for r in resid)),
                note="these points did not influence the fitted coefficients")


def _cond(status, reason):
    return dict(status=status, reason=reason)


def conditions(rec, out, blockers, unresolved) -> dict:
    """The explicit list of required evidence and model-adequacy conditions.

    Every entry here gates CALIBRATION_USABLE. A diagnostic that is computed but not listed here
    cannot silently fail to matter, which was the defect this replaces."""
    c = {}
    c["no_fatal_blockers"] = _cond("pass" if not any(b["fatal"] for b in blockers) else "fail",
                                   "fatal evidence blockers prevent a fit")
    adv = [b["blocker"] for b in blockers if not b["fatal"]]
    c["no_advisory_blockers"] = _cond("pass" if not adv else "fail", "; ".join(adv) or "none")

    fq = out.get("fit_quality", {})
    v = fq.get("verdict")
    c["model_adequate"] = _cond(
        {"LACK_OF_FIT_NOT_SIGNIFICANT": "pass", "LACK_OF_FIT_SIGNIFICANT": "fail"}.get(v, "unevaluable"),
        f"lack-of-fit verdict {v}: {fq.get('reason')}")

    z = out.get("zero_drift", {})
    if z.get("force_N") is None:
        bounded = rec.get("zero_drift_declared_negligible")
        c["zero_observations"] = _cond("pass" if bounded else "unevaluable",
                                       f"zero readings missing; declared negligible with basis: {bounded}"
                                       if bounded else
                                       "zero_start_counts / zero_end_counts absent, so zero return is "
                                       "unmeasured. Supply them, or set zero_drift_declared_negligible "
                                       "to a stated basis. Missing data is not a small value.")
    else:
        c["zero_observations"] = _cond("pass", "zero return measured")

    ug = rec.get("u_g_m_s2")
    basis = rec.get("u_g_declared_negligible")
    c["gravity_uncertainty"] = _cond(
        "pass" if (ug is not None and ug > 0) or basis else "unevaluable",
        "u_g_m_s2 absent and not declared negligible; a defaulted zero is an assumption, not a datum"
        if not basis else f"declared negligible with basis: {basis}")

    lo, hi = out.get("calibrated_force_range_N", (None, None))
    need = (min(CAMPAIGN_FORCES_N), max(CAMPAIGN_FORCES_N))
    c["covers_campaign_range"] = _cond(
        "pass" if lo is not None and lo <= need[0] and hi >= need[1] else "fail",
        f"calibrated {lo}..{hi} N against required {need[0]}..{need[1]} N")

    c["identity_complete"] = _cond("pass" if not unresolved else "unevaluable",
                                   f"unresolved identity fields: {unresolved}")

    ho = out.get("held_out", {})
    c["held_out_scored"] = _cond("pass" if ho.get("status") == "present" else "unevaluable",
                                 ho.get("note", ""))

    c["reviewed"] = _cond("pass" if rec.get("reviewer_state") == "reviewed" else "unevaluable",
                          f"reviewer_state is {rec.get('reviewer_state')!r}; a calibration nobody "
                          f"reviewed is not a reviewed calibration")
    return c
